...
def basic_tokenisation(text):
    sentences = re.split('[.!?]', text)
    return sentences

def dynamic_max_df(sentences, base_threshold=0.85, min_threshold=0.5):
...
def tf_idf_keywords(user_text):
    ...
    sentences = basic_tokenisation(user_text)  # split into words
    max_df = dynamic_max_df(sentences)
    ...
    return remove_redundant_keywords(list(all_top_words))
